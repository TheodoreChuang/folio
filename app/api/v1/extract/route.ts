...
export async function POST(request: Request) {
  ...
  if (resolved.authMethod === 'bearer') {
    return NextResponse.json(
      { error: 'PDF extraction requires session authentication. Use the Folio web app to upload and extract documents.' },
      { status: 400 }
    )
  }
  const user = resolved
  ...
  if (downloadError || !data) {
    captureError(downloadError ?? new Error('no data'), { route: 'POST /api/v1/extract', phase: 'storage-download' })
    return NextResponse.json(
      {
  ...
  if (doc.documentType === 'pm_statement' || doc.documentType === 'loan_statement') {
    ...
    try {
      ...
      captureError(err, { route: 'POST /api/v1/extract', phase: 'classification' })
    ...
    try {
      ...
      clearTimeout(timeoutId)
      captureError(err, { route: 'POST /api/v1/extract', phase: 'type-update' })
      const message = err instanceof Error ? err.message : String(err)
  ...
  if (documentType === 'loan_statement') {
    ...
    try {
      ...
      captureError(err, { route: 'POST /api/v1/extract', phase: 'ai-extraction' })
      const message = err instanceof Error ? err.message : String(err)
      return NextResponse.json(
    ...
    try {
      ...
      clearTimeout(timeoutId)
      captureError(err, { route: 'POST /api/v1/extract', phase: 'staging' })
      const message = err instanceof Error ? err.message : String(err)
      return NextResponse.json(
  ...
  clearTimeout(timeoutId)
  captureError(new Error(`Unexpected document type: ${documentType}`), { route: 'POST /api/v1/extract' })
