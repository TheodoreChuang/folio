...
import { commitLoanStagedItems } from '@/lib/ingestion'
...
export async function POST(request: Request) {
  try {
    ...
    try {
      result = await commitLoanStagedItems(user.id, sourceDocumentIds)
    } catch (err) {
    ...
    captureError(err, { route: 'POST /api/v1/ingestion/loan-commit' })
