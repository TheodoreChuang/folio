...
export function computeTrends(rows: TrendRow[], months: string[]): TrendPoint[] {
  type Bucket = { rent: number; income: number; expenses: number; mortgage: number }
  const buckets = new Map<string, Bucket>()
  for (const row of rows) {
    const b = buckets.get(row.month) ?? { rent: 0, income: 0, expenses: 0, mortgage: 0 }
    const bucket = CATEGORY_BUCKET[row.category]
    if (bucket === 'rent') b.rent += Number(row.totalCents)
    else if (bucket === 'income') b.income += Number(row.totalCents)
    else if (bucket === 'expense') b.expenses += Number(row.totalCents)
    else b.mortgage += Number(row.totalCents)
    buckets.set(row.month, b)
  }
  return months.map(month => {
    const b = buckets.get(month) ?? { rent: 0, income: 0, expenses: 0, mortgage: 0 }
    return {
      month,
      rentCents:        b.rent,
      otherIncomeCents: b.income,
      expensesCents:    b.expenses,
      mortgageCents:    b.mortgage,
      netCents:         b.rent + b.income - b.expenses - b.mortgage,
      hasData:          b.rent > 0 || b.income > 0 || b.expenses > 0 || b.mortgage > 0,
    }
  })
}
